...
describe("getPackageName", () => {
	it("maps win32 platform to windows package", () => {
		expect(getPackageName("win32", "x64")).toBe("mini-backlog.md-windows-x64");
	});

	it("maps win32 arm64 to windows-arm64 package", () => {
		expect(getPackageName("win32", "arm64")).toBe("mini-backlog.md-windows-arm64");
	});

	it("returns linux name unchanged", () => {
		expect(getPackageName("linux", "arm64")).toBe("mini-backlog.md-linux-arm64");
	});

	it("maps darwin arches to darwin packages", () => {
		expect(getPackageName("darwin", "arm64")).toBe("mini-backlog.md-darwin-arm64");
		expect(getPackageName("darwin", "x64")).toBe("mini-backlog.md-darwin-x64");
	});
});
...
describe("getCandidatePackageNames", () => {
	...
	it("does not add fallbacks on other platforms", () => {
		expect(getCandidatePackageNames("linux", "x64")).toEqual(["mini-backlog.md-linux-x64"]);
		expect(getCandidatePackageNames("linux", "arm64")).toEqual(["mini-backlog.md-linux-arm64"]);
		expect(getCandidatePackageNames("win32", "x64")).toEqual(["mini-backlog.md-windows-x64"]);
	});

	it("does not add fallbacks for unknown darwin arches", () => {
		expect(getCandidatePackageNames("darwin", "ppc64")).toEqual(["mini-backlog.md-darwin-ppc64"]);
	});
});

describe("resolveBinaryPath", () => {
	it("resolves the native package when it is installed", () => {
		const resolver = resolverFor(["mini-backlog.md-darwin-arm64/backlog", "mini-backlog.md-darwin-x64/backlog"]);
		expect(resolveBinaryPath("darwin", "arm64", resolver)).toBe("/node_modules/mini-backlog.md-darwin-arm64/backlog");
	});

	it("falls back to darwin-x64 when arm64 Node only has the x64 package", () => {
		const resolver = resolverFor(["mini-backlog.md-darwin-x64/backlog"]);
		expect(resolveBinaryPath("darwin", "arm64", resolver)).toBe("/node_modules/mini-backlog.md-darwin-x64/backlog");
	});

	it("falls back to darwin-arm64 when Rosetta x64 Node only has the arm64 package", () => {
		const resolver = resolverFor(["mini-backlog.md-darwin-arm64/backlog"]);
		expect(resolveBinaryPath("darwin", "x64", resolver)).toBe("/node_modules/mini-backlog.md-darwin-arm64/backlog");
	});
	...
	it("throws the original error when no darwin package is installed", () => {
		expect(() => resolveBinaryPath("darwin", "arm64", resolverFor([]))).toThrow(
			"Cannot find module 'mini-backlog.md-darwin-arm64/backlog'",
		);
	});

	it("does not fall back across arches on linux", () => {
		const resolver = resolverFor(["mini-backlog.md-linux-arm64/backlog"]);
		expect(() => resolveBinaryPath("linux", "x64", resolver)).toThrow(
			"Cannot find module 'mini-backlog.md-linux-x64/backlog'",
		);
	});

	it("resolves the .exe binary on windows", () => {
		const resolver = resolverFor(["mini-backlog.md-windows-x64/backlog.exe"]);
		expect(resolveBinaryPath("win32", "x64", resolver)).toBe("/node_modules/mini-backlog.md-windows-x64/backlog.exe");
	});
});
